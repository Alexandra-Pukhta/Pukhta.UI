...
{
    public class ApiCarService(HttpClient httpClient) : IProductService
    {
        public async Task<ResponseData<Car>> CreateProductAsync(Car product, IFormFile? formFile)
        {
            ...
            var responseData = new ResponseData<Car>();
            ...
            if (formFile != null)
            {
                ...
                var Tovar = await response.Content.ReadFromJsonAsync<Car>();
        ...
        public async Task<ResponseData<Car>> GetProductByIdAsync(int id)
        {
            ...
            var response = await httpClient.GetFromJsonAsync<Car>(apiUrl);
            return new ResponseData<Car>() { Data = response };
        }
